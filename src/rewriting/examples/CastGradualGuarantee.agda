...
𝒱-dyn-any-elim : ∀{𝒫}{A′}{V}{V′}{R}
   ...
   → 𝒫 ⊢ᵒ R
𝒱-dyn-any-elim {𝒫}{A′}{V}{V′}{R} ⊢𝒱VV′ cont = {!!}
...
compatible-proj-L {Γ}{H}{A′}{c}{M}{M′} ⊨M⊑M′ γ γ′ =
  ...
  𝒫₁ = λ V V′ → 𝒱⟦ ★ , A′ , unk⊑ ⟧ V V′
               ∷ (⟪ γ′ ⟫ M′ —↠ V′)ᵒ ∷ (⟪ γ ⟫ M —↠ V)ᵒ ∷ 𝓖⟦ Γ ⟧ γ γ′
  ⊢ℰV⟨H?⟩V′ : ∀{V V′} → 𝒫₁ V V′ ⊢ᵒ  ℰ⟦ gnd⇒ty H , A′ , c ⟧ (V ⟨ H ?⟩) V′
  ⊢ℰV⟨H?⟩V′ {V}{V′} = 𝒱-dyn-any-elim Zᵒ Goal
    where
    Goal : ∀{V₁} {G} → V ≡ (V₁ ⟨ G !⟩) → (c₁ : gnd⇒ty G ⊑ A′)
       → 𝒫₁ V V′ ⊢ᵒ Value V₁ ᵒ ×ᵒ Value V′ ᵒ
                     ×ᵒ (▷ᵒ 𝒱⟦ gnd⇒ty G , A′ , c₁ ⟧ V₁ V′)
       → 𝒫₁ V V′ ⊢ᵒ ℰ⟦ gnd⇒ty H , A′ , c ⟧ (V ⟨ H ?⟩) V′
    Goal {V₁} {G} refl ⊢v₁×v′×▷𝒱V₁V′ = {!!}
