...
blame? (M ⟨ G !⟩) = no λ ()
blame? (M ⟨ H ?⟩) = no λ ()
blame? blame = yes isBlame

return : Term → Maybe Result
return H = just (halt H)
...
letB-halt-inv : ∀ (M : Maybe Result){f : Term → Maybe Result}{W : Term}
   → letB M f ≡ return W
   → (∃[ V ] (V ≢ blame) × (M ≡ return V) × (f V ≡ return W))
     ⊎ (M ≡ return blame × W ≡ blame)
...
-- -- eval-app-blame-L : ∀{L M}
-- --    → L ⇓ᵇ
-- --    → (L · M) ⇓ᵇ
-- -- eval-app-blame-L {L}{M} (k , evL=bl) = (suc k) , Goal evL=bl
-- --   where
-- --   Goal : eval L k ≡ blame!
-- --      → (letᵇ V := (eval L k) ; letᵇ W := (eval M k) ;
-- --         letᵐ N := (V ⊙ W) ; eval N k)  ≡  blame!
-- --   Goal evL=bl rewrite evL=bl = refl

-- -- eval-app-blame-R : ∀{L V M}
-- --    → L ⇓ V
-- --    → M ⇓ᵇ
-- --    → (L · M) ⇓ᵇ
-- -- eval-app-blame-R {L}{V}{M}  = ?

-- -- eval-app-blame-inv : ∀{L M}
-- --    → (L · M) ⇓ᵇ
-- --    → L ⇓ᵇ
-- --      ⊎ (∃[ V ] L ⇓ V × M ⇓ᵇ)
-- --      ⊎ (∃[ N ] ∃[ W ] L ⇓ (ƛ N) × M ⇓ W × N [ W ] ⇓ᵇ)
-- -- eval-app-blame-inv {L} {M} (suc k , evalLM=bl)
-- --     with letB-halt-inv (eval L k) evalLM=bl
-- -- ... | inj₂ (evL=bl , _) =
-- --       inj₁ (k , evL=bl)
-- -- ... | inj₁ (V , (evL=V , eqLetM))
-- --     with letM-just-inv (eval M k) eqLetM
-- -- ... | halt hblame , evM=R , eq2 rewrite evM=R =
-- --       inj₂ (inj₁ (V , (k , evL=V) , k , evM=R))
-- -- ... | timeout , evM=R , ()
-- -- ... | halt (` W) , evM=W , eq2 rewrite evM=W
-- --     with eval-to-value L V k evL=V
-- -- ... | ƛ̬ N =
-- --       inj₂ (inj₂ (N , (W , ((k , evL=V) , ((k , evM=W) , (k , eq2))))))
