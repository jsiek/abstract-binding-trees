...
compatible-app {Γ}{↪}{A}{A′}{B}{B′}{c}{d}{L}{L′}{M}{M′}
 ...
 Bl = →ᵒI (⊢ᵒ-sucP Zᵒ (λ LM⇓blame → G LM⇓blame))
  ...
  ... | app⇓{N = N}{W} L⇓λN M⇓W w NW⇓blame =
      let F₁ = λ V → (Value V)ᵒ →ᵒ ((⟪ γ ⟫ L) ⇓ V)ᵒ
                     →ᵒ ((∃ᵒ[ V′ ] (Value V′)ᵒ ×ᵒ ((⟪ γ′ ⟫ L′) ⇓ V′)ᵒ
                         ×ᵒ 𝒱⟦ A ⇒ B , A′ ⇒ B′ , fun⊑ c d ⟧ ↪ V V′)
                      ⊎ᵒ ((⟪ γ′ ⟫ L′) ⇓ blame)ᵒ) in
      let F = λ V′ → (Value V′)ᵒ ×ᵒ ((⟪ γ′ ⟫ L′) ⇓ V′)ᵒ
                     ×ᵒ 𝒱⟦ A ⇒ B , A′ ⇒ B′ , fun⊑ c d ⟧ ↪ (ƛ N) V′ in
      let ⊢[∃V′v′×L′⇓V′×𝒱λNV′]⊎L′⇓blame :
            𝒫₁ ⊢ᵒ (∃ᵒ[ V′ ] F V′) ⊎ᵒ ((⟪ γ′ ⟫ L′) ⇓ blame)ᵒ 
          ⊢[∃V′v′×L′⇓V′×𝒱λNV′]⊎L′⇓blame =
               Sᵒ (appᵒ (appᵒ (instᵒ{P = F₁} (proj₁ᵒ
                          (substᵒ ℰ↪-stmt (⊨L⊑L′ γ γ′)))
                          (ƛ N)) (constᵒI (ƛ̬ N))) (constᵒI L⇓λN)) in
      caseᵒ ⊢[∃V′v′×L′⇓V′×𝒱λNV′]⊎L′⇓blame
        (let 𝒫₂ = (∃ᵒ[ V′ ] F V′) ∷ 𝒫₁ in
         let ∃V′ : 𝒫₂ ⊢ᵒ (∃ᵒ[ V′ ] F V′)
             ∃V′ = Zᵒ in
         ⊢ᵒ-∃-elim{P = F} ∃V′ λ V′ →
         let 𝒫₃ = F V′ ∷ 𝒫₂ in
         let ⊢𝒱λNV′ : 𝒫₃ ⊢ᵒ 𝒱⟦ A ⇒ B , A′ ⇒ B′ , fun⊑ c d ⟧ ↪ (ƛ N) V′
             ⊢𝒱λNV′ = (proj₂ᵒ (proj₂ᵒ Zᵒ)) in
         𝒱-fun-elim ⊢𝒱λNV′ λ {N N′ refl refl body →
         let ⊢L′⇓V′ : 𝒫₃ ⊢ᵒ ((⟪ γ′ ⟫ L′) ⇓ V′)ᵒ
             ⊢L′⇓V′ = proj₁ᵒ (proj₂ᵒ Zᵒ) in
         constᵒE ⊢L′⇓V′ λ L′⇓V′ →
         let F₂ = λ W′ → (Value W′)ᵒ ×ᵒ ((⟪ γ′ ⟫ M′) ⇓ W′)ᵒ
                         ×ᵒ 𝒱⟦ A , A′ , c ⟧ ↪ W W′ in
         let ⊢[∃W′w′×M′⇓W′×𝒱WW′]⊎M′⇓blame :
               𝒫₃ ⊢ᵒ (∃ᵒ[ W′ ] F₂ W′) ⊎ᵒ ((⟪ γ′ ⟫ M′) ⇓ blame)ᵒ
             ⊢[∃W′w′×M′⇓W′×𝒱WW′]⊎M′⇓blame =
               Sᵒ (Sᵒ (Sᵒ (appᵒ (appᵒ (instᵒ (proj₁ᵒ
                          (substᵒ ℰ↪-stmt (⊨M⊑M′ γ γ′)))
                          W) (constᵒI w)) (constᵒI M⇓W)))) in
         
         caseᵒ ⊢[∃W′w′×M′⇓W′×𝒱WW′]⊎M′⇓blame
         (let 𝒫₄ = (∃ᵒ[ W′ ] F₂ W′) ∷ 𝒫₃ in
          let ∃W′ : 𝒫₄ ⊢ᵒ (∃ᵒ[ W′ ] F₂ W′)
              ∃W′ = Zᵒ in
          ⊢ᵒ-∃-elim{P = F₂} ∃W′ λ W′ →
         ...
         (⊢ᵒ-sucP Zᵒ λ M′⇓blame →
           ⊢ᵒ-sucP (proj₁ᵒ (proj₂ᵒ (Sᵒ Zᵒ))) λ L′⇓λN′ →
             constᵒI (app⇓-blame-R L′⇓λN′ M′⇓blame))
         })
        (⊢ᵒ-sucP Zᵒ λ L′⇓blame → constᵒI (app⇓-blame-L L′⇓blame))
