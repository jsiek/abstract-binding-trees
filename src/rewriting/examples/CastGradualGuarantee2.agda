  ...
  G = Λᵒ[ V₁ ] (→ᵒI{P = (V ⇓ V₁)ᵒ}
      ...
      (inj₁ᵒ (⊢ᵒ-∃-intro{P = F} V′ (constᵒI (⇓-value V′ v′)
                ,ᵒ subst (λ X → (V ⇓ V₁)ᵒ ∷ 𝒫 ⊢ᵒ 𝒱⟦ c ⟧ ↪ X V′)
                         ...
                         (⊢ᵒ-weaken ⊢𝒱VV′)))))))))
  B : 𝒫 ⊢ᵒ (V ⇓ᵇ)ᵒ →ᵒ (V′ ⇓ᵇ)ᵒ
  B = →ᵒI{P = (V ⇓ᵇ)ᵒ} (⊢ᵒ-sucP Zᵒ λ V⇓ᵇ →
      (⊢ᵒ-sucP (⊢ᵒ-weaken ⊢𝒱VV′) (λ 𝒱VV′ →
      let (v , v′) = 𝒱⇒Value c V V′ 𝒱VV′ in
      ⊥-elim (⇓ᵇ-not-value V⇓ᵇ v))))

  D : 𝒫 ⊢ᵒ (V ⇑)ᵒ →ᵒ (V′ ⇑)ᵒ ⊎ᵒ (V′ ⇓ᵇ)ᵒ
...
ℰ-blame {𝒫} {c} {↪} {M} = substᵒ (≡ᵒ-sym ℰ↪-stmt) (G ,ᵒ (B ,ᵒ D))
  ...
  G : 𝒫 ⊢ᵒ (∀ᵒ[ V₁ ]  (M ⇓ V₁)ᵒ
      →ᵒ ((∃ᵒ[ V′₁ ] (blame ⇓ V′₁)ᵒ ×ᵒ 𝒱⟦ c ⟧ ↪ V₁ V′₁)
         ⊎ᵒ (blame ⇓ᵇ)ᵒ))
  G = Λᵒ[ V₁ ] (→ᵒI{P = (M ⇓ V₁)ᵒ} (inj₂ᵒ (constᵒI ⇓ᵇ-blame)))

  B : 𝒫 ⊢ᵒ (M ⇓ᵇ)ᵒ →ᵒ (blame ⇓ᵇ)ᵒ
  B = →ᵒI{P = (M ⇓ᵇ)ᵒ} (constᵒI ⇓ᵇ-blame)

  D : 𝒫 ⊢ᵒ (M ⇑)ᵒ →ᵒ (blame ⇑)ᵒ ⊎ᵒ (blame ⇓ᵇ)ᵒ
  D = →ᵒI{P = (M ⇑)ᵒ} (inj₂ᵒ (constᵒI ⇓ᵇ-blame))
