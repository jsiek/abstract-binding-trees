...
𝒱↪-timeout {𝒫}{V}{V′} ⊢𝒱VV′ = ⊢ᵒ-intro λ { zero (V⇑ , ⊨𝒫n) → tt}
...
𝒱⇒ℰ-aux : ∀{d}{𝒫}
   → 𝒫 ⊢ᵒ 𝒱⇒ℰ-pred d
𝒱⇒ℰ-aux{d}{𝒫} =
  let F₁ = λ V → ∀ᵒ[ V′ ] ∀ᵒ[ c ] (𝒱⟦ c ⟧ d V V′) →ᵒ (ℰ⟦ c ⟧ d V V′) in
  let F₂ = λ V V′ → ∀ᵒ[ c ] (𝒱⟦ c ⟧ d V V′) →ᵒ (ℰ⟦ c ⟧ d V V′) in
  let F₃ = λ V V′ c → (𝒱⟦ c ⟧ d V V′) →ᵒ (ℰ⟦ c ⟧ d V V′) in
  lobᵒ (⊢ᵒ-∀-intro-new F₁ λ V → ⊢ᵒ-∀-intro-new (F₂ V) λ V′ →
    ⊢ᵒ-∀-intro-new (F₃ V V′) λ c →
    (→ᵒI
    (⊢ᵒ-sucP Zᵒ λ 𝒱VV′sn →
  ...
  Goal {V} {V′} {.★ , A′ , unk⊑} {d} {n} 𝒱VV′ v v′
      with dyn? A′
  ... | yes refl =
        𝒱-dyn-dyn-elim Zᵒ 𝒱VV′ λ {V₁ V′₁ G v₁ v′₁ refl refl ⊢▷𝒱V₁V′₁ →
        let c′ = gnd⇒ty G , gnd⇒ty G , Refl⊑ in
        let xx : 𝒫₁ ⊢ᵒ ∀ᵒ[ c′ ] (▷ᵒ (𝒱⟦ c′ ⟧ d V₁ V′₁ →ᵒ ℰ⟦ c′ ⟧ d V₁ V′₁))
            xx = ▷∀ (instᵒ (▷∀ (instᵒ (▷∀ (Sᵒ Zᵒ)) V₁)) V′₁) in
        let F = λ c′ → (▷ᵒ (𝒱⟦ c′ ⟧ d V₁ V′₁ →ᵒ ℰ⟦ c′ ⟧ d V₁ V′₁)) in
        let ▷ℰV₁V′₁ : 𝒫₁ ⊢ᵒ ▷ᵒ ℰ⟦ c′ ⟧ d V₁ V′₁
            ▷ℰV₁V′₁ = appᵒ (▷→ (instᵒ{P = F} xx c′)) ⊢▷𝒱V₁V′₁ in
        Goal2 ▷ℰV₁V′₁}
        where
        𝒫₁ = 𝒱⟦ ★ , A′ , unk⊑ ⟧ d V V′ ∷ ▷ᵒ 𝒱⇒ℰ-pred d ∷ 𝒫
        Goal2 : ∀{V₁}{V′₁}{G}{d}
           → 𝒫₁ ⊢ᵒ ▷ᵒ ℰ⟦ gnd⇒ty G , gnd⇒ty G , Refl⊑ ⟧ d V₁ V′₁
           → 𝒫₁ ⊢ᵒ ℰ⟦ ★ , ★ , unk⊑ ⟧ d (V₁ ⟨ G !⟩) (V′₁ ⟨ G !⟩)
        Goal2 {V₁} {V′₁} {G} {↪} ▷ℰV₁V′₁ = substᵒ (≡ᵒ-sym ℰ↪-stmt)
           (Λᵒ[ R ] (→ᵒI {!!}))
        Goal2 {V₁} {V′₁} {G} {↩} ▷ℰV₁V′₁ = {!!}
  ... | no A′nd = {!!}
