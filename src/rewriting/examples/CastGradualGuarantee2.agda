...
compatible-app {Γ}{↪}{A}{A′}{B}{B′}{c}{d}{L}{L′}{M}{M′}
 ...
 Bl = →ᵒI (⊢ᵒ-sucP Zᵒ (λ LM⇓blame → G LM⇓blame))
  ...
  ... | app⇓{N = N}{W} L⇓λN M⇓W w NW⇓blame =
      let c→d = A ⇒ B , A′ ⇒ B′ , fun⊑ c d in
      let F = v′×⇓×𝒱 (ƛ N) c→d (⟪ γ′ ⟫ L′) in
      let ⊢[∃V′v′×L′⇓V′×𝒱λNV′]⊎L′⇓blame :
            𝒫₁ ⊢ᵒ (∃ᵒ[ V′ ] F V′) ⊎ᵒ ((⟪ γ′ ⟫ L′) ⇓ blame)ᵒ 
          ⊢[∃V′v′×L′⇓V′×𝒱λNV′]⊎L′⇓blame =
               Sᵒ (appᵒ (appᵒ (instᵒ{P = ↪⇓𝒱 c→d (⟪ γ ⟫ L) (⟪ γ′ ⟫ L′)}
                          (proj₁ᵒ (substᵒ ℰ↪-stmt (⊨L⊑L′ γ γ′)))
      ...
      caseᵒ ⊢[∃V′v′×L′⇓V′×𝒱λNV′]⊎L′⇓blame
        (let 𝒫₂ = (∃ᵒ[ V′ ] F V′) ∷ 𝒫₁ in
         ...
         let ⊢M′⇓blame : 𝒫₃ ⊢ᵒ ((⟪ γ′ ⟫ M′) ⇓ blame)ᵒ 
             ⊢M′⇓blame = Sᵒ (Sᵒ (Sᵒ (appᵒ (proj₁ᵒ (proj₂ᵒ 
                                      (substᵒ ℰ↪-stmt (⊨M⊑M′ γ γ′))))
                                    (constᵒI M⇓blame)))) in
         constᵒE ⊢M′⇓blame λ M′⇓blame →
         constᵒI (app⇓-blame-R L′⇓V′ M′⇓blame)
        })
        (⊢ᵒ-sucP Zᵒ λ L′⇓blame → constᵒI (app⇓-blame-L L′⇓blame))
