...
pre-ℰ c ↪ M M′ =
      (∀ˢ[ V ]  (M ⇓ V)ˢ →ˢ ((∃ˢ[ V′ ] (M′ ⇓ V′)ˢ ×ˢ pre-𝒱 c ↪ V V′)
                            ⊎ˢ (M′ ⇓ blame)ˢ))
...
ℰ↪-stmt : ∀{c}{M M′}
  → ℰ⟦ c ⟧ ↪ M M′ ≡ᵒ
        ((∀ᵒ[ V ]  (M ⇓ V)ᵒ →ᵒ ((∃ᵒ[ V′ ] (M′ ⇓ V′)ᵒ ×ᵒ 𝒱⟦ c ⟧ ↪ V V′)
                            ⊎ᵒ (M′ ⇓ blame)ᵒ))
...
ℰ↪-stmt {c}{M}{M′} =
  ...
  # (pre-ℰ⊎𝒱 X₂) (ℰ⊎𝒱 , ttᵖ)
      ⩦⟨ (cong-×ᵒ (cong-∀ λ V → cong-→{S = (M ⇓ V)ᵒ} (≡ᵒ-refl refl) (cong-⊎ᵒ (cong-∃ (λ V′ → cong-×ᵒ (≡ᵒ-refl refl) (≡ᵒ-sym (fixpointᵒ pre-ℰ⊎𝒱 (inj₁ (c , ↪ , V , V′)))))) (≡ᵒ-refl refl))) (≡ᵒ-refl refl)) ⟩
      ((∀ᵒ[ V ]  (M ⇓ V)ᵒ →ᵒ ((∃ᵒ[ V′ ] (M′ ⇓ V′)ᵒ ×ᵒ 𝒱⟦ c ⟧ ↪ V V′)
                            ⊎ᵒ (M′ ⇓ blame)ᵒ))
   ×ᵒ ((M ⇓ blame)ᵒ →ᵒ (M′ ⇓ blame)ᵒ)
   ×ᵒ ((M ⇑)ᵒ →ᵒ ((M′ ⇑)ᵒ ⊎ᵒ (M′ ⇓ blame)ᵒ)))           ∎
...
ℰ↩-stmt {c}{M}{M′} =
  ...
  μᵒ pre-ℰ⊎𝒱 X₂                                     ⩦⟨ fixpointᵒ pre-ℰ⊎𝒱 X₂ ⟩
  # (pre-ℰ⊎𝒱 X₂) (ℰ⊎𝒱 , ttᵖ)
      ⩦⟨ cong-×ᵒ (cong-∀ λ V′ → cong-→{S = (M′ ⇓ V′)ᵒ} (≡ᵒ-refl refl) (cong-∃ λ V → cong-×ᵒ (≡ᵒ-refl refl) (≡ᵒ-sym (fixpointᵒ pre-ℰ⊎𝒱 (inj₁ (c , ↩ , V , V′)))))) (≡ᵒ-refl refl) ⟩
      ((∀ᵒ[ V′ ] (M′ ⇓ V′)ᵒ →ᵒ (∃ᵒ[ V ] (M ⇓ V)ᵒ ×ᵒ 𝒱⟦ c ⟧ ↩ V V′))
   ×ᵒ ((M′ ⇑)ᵒ →ᵒ (M ⇑)ᵒ))            ∎
...
𝒱⇒ℰ {c}{↪}{𝒫}{V}{V′} ⊢𝒱VV′ = substᵒ (≡ᵒ-sym ℰ↪-stmt) 
  (G ,ᵒ (B ,ᵒ D))
  where
  G : 𝒫 ⊢ᵒ (∀ᵒ[ V₁ ]  (V ⇓ V₁)ᵒ →ᵒ ((∃ᵒ[ V′₁ ] (V′ ⇓ V′₁)ᵒ ×ᵒ 𝒱⟦ c ⟧ ↪ V₁ V′₁)
             ⊎ᵒ (V′ ⇓ blame)ᵒ))
  G = Λᵒ[ V₁ ] (→ᵒI{P = (V ⇓ V₁)ᵒ}
      (⊢ᵒ-sucP (⊢ᵒ-weaken ⊢𝒱VV′) (λ 𝒱VV′ →
      (⊢ᵒ-sucP Zᵒ (λ V⇓V₁ →
      let (v , v′) = 𝒱⇒Value c V V′ 𝒱VV′ in
      let F = (λ V′₁ → (V′ ⇓ V′₁)ᵒ ×ᵒ 𝒱⟦ c ⟧ ↪ V₁ V′₁) in
      (inj₁ᵒ (⊢ᵒ-∃-intro{P = F} V′ (constᵒI (⇓-value V′ v′)
                ,ᵒ subst (λ X → (V ⇓ V₁)ᵒ ∷ 𝒫 ⊢ᵒ 𝒱⟦ c ⟧ ↪ X V′)
                         (sym (⇓-value-eq v V⇓V₁))
                         (⊢ᵒ-weaken ⊢𝒱VV′)))))))))
  B : 𝒫 ⊢ᵒ (V ⇓ blame)ᵒ →ᵒ (V′ ⇓ blame)ᵒ
  B = →ᵒI{P = (V ⇓ blame)ᵒ} (⊢ᵒ-sucP Zᵒ λ V⇓ blame →
      (⊢ᵒ-sucP (⊢ᵒ-weaken ⊢𝒱VV′) (λ 𝒱VV′ →
      let (v , v′) = 𝒱⇒Value c V V′ 𝒱VV′ in
      ⊥-elim (⇓ blame-not-value V⇓ blame v))))
...
𝒱⇒ℰ {c}{↩}{𝒫}{V}{V′} ⊢𝒱VV′ = substᵒ (≡ᵒ-sym ℰ↩-stmt) (G ,ᵒ D)
  where
  G : 𝒫 ⊢ᵒ (∀ᵒ[ V′₁ ] (V′ ⇓ V′₁)ᵒ
             →ᵒ (∃ᵒ[ V₁ ] (V ⇓ V₁)ᵒ ×ᵒ 𝒱⟦ c ⟧ ↩ V₁ V′₁))
  G = Λᵒ[ V′₁ ] →ᵒI{P = (V′ ⇓ V′₁)ᵒ}
      (⊢ᵒ-sucP (⊢ᵒ-weaken ⊢𝒱VV′) (λ 𝒱VV′ →
      (⊢ᵒ-sucP Zᵒ (λ V′⇓V′₁ →
      let (v , v′) = 𝒱⇒Value c V V′ 𝒱VV′ in
      let xx = subst (λ X → (V′ ⇓ V′₁)ᵒ ∷ 𝒫 ⊢ᵒ 𝒱⟦ c ⟧ ↩ V X)
                         (sym (⇓-value-eq v′ V′⇓V′₁))
                         (⊢ᵒ-weaken ⊢𝒱VV′) in
      let F = (λ V₁ → (V ⇓ V₁)ᵒ ×ᵒ 𝒱⟦ c ⟧ ↩ V₁ V′₁) in
      (⊢ᵒ-∃-intro{P = F} V (constᵒI (⇓-value V v) ,ᵒ xx))
      ))))
...
ℰ-blame {𝒫} {c} {↪} {M} = substᵒ (≡ᵒ-sym ℰ↪-stmt) (G ,ᵒ (B ,ᵒ D))
  where
  G : 𝒫 ⊢ᵒ (∀ᵒ[ V₁ ]  (M ⇓ V₁)ᵒ
      →ᵒ ((∃ᵒ[ V′₁ ] (blame ⇓ V′₁)ᵒ ×ᵒ 𝒱⟦ c ⟧ ↪ V₁ V′₁)
         ⊎ᵒ (blame ⇓ blame)ᵒ))
  G = Λᵒ[ V₁ ] (→ᵒI{P = (M ⇓ V₁)ᵒ} (inj₂ᵒ (constᵒI ⇓ blame-blame)))

  B : 𝒫 ⊢ᵒ (M ⇓ blame)ᵒ →ᵒ (blame ⇓ blame)ᵒ
  B = →ᵒI{P = (M ⇓ blame)ᵒ} (constᵒI ⇓ blame-blame)

  D : 𝒫 ⊢ᵒ (M ⇑)ᵒ →ᵒ (blame ⇑)ᵒ ⊎ᵒ (blame ⇓ blame)ᵒ
  D = →ᵒI{P = (M ⇑)ᵒ} (inj₂ᵒ (constᵒI ⇓ blame-blame))

ℰ-blame {𝒫} {c} {↩} {M} = substᵒ (≡ᵒ-sym ℰ↩-stmt) (G ,ᵒ D)
  where
  G : 𝒫 ⊢ᵒ (∀ᵒ[ V′₁ ] (blame ⇓ V′₁)ᵒ
             →ᵒ (∃ᵒ[ V₁ ] (M ⇓ V₁)ᵒ ×ᵒ 𝒱⟦ c ⟧ ↩ V₁ V′₁))
  G = Λᵒ[ V′₁ ] →ᵒI{P = (blame ⇓ V′₁)ᵒ} (⊢ᵒ-sucP Zᵒ λ blame⇓ →
          ⊥-elim (blame-eval-not-value blame⇓))
...
compatible-app {Γ}{↪}{A}{A′}{B}{B′}{c}{d}{L}{L′}{M}{M′}
 ...
 where
 Bl : 𝓖⟦ Γ ⟧ ↪ γ γ′ ⊢ᵒ ((⟪ γ ⟫ (L · M)) ⇓ blame)ᵒ →ᵒ ((⟪ γ′ ⟫ (L′ · M′)) ⇓ blame)ᵒ
 Bl = →ᵒI (⊢ᵒ-sucP Zᵒ (λ LM⇓ blame → G LM⇓ blame))
  where
  G : ⟪ γ ⟫ (L · M) ⇓ blame
     → ((⟪ γ ⟫ (L · M)) ⇓ blame)ᵒ ∷ 𝓖⟦ Γ ⟧ ↪ γ γ′ ⊢ᵒ ((⟪ γ′ ⟫ (L′ · M′)) ⇓ blame)ᵒ
  G LM⇓ blame
     with eval-app-blame-inv LM⇓ blame
  ... | inj₁ L⇓ blame =
        let ⊢L′⇓ blame = Sᵒ (appᵒ (proj₁ᵒ (proj₂ᵒ (substᵒ ℰ↪-stmt (⊨L⊑L′ γ γ′))))
                       (constᵒI L⇓ blame)) in
        ⊢ᵒ-sucP ⊢L′⇓ blame λ L′⇓ blame → constᵒI (eval-app-blame-L{M = ⟪ γ′ ⟫ M′} L′⇓ blame)
  ... | inj₂ (inj₁ (V , L⇓V , M⇓ blame)) =
        let xx = (appᵒ (instᵒ (proj₁ᵒ (substᵒ ℰ↪-stmt (⊨L⊑L′ γ γ′))) V)
                              (constᵒI L⇓V)) in
        let ⊢L′⇓V′ = Sᵒ (proj₁ᵒ xx) in
        let ⊢M′⇓ blame = Sᵒ (appᵒ (proj₁ᵒ (proj₂ᵒ (substᵒ ℰ↪-stmt (⊨M⊑M′ γ γ′))))
                              (constᵒI M⇓ blame)) in
        {!!}
  ... | inj₂ (inj₂ (N , W , L⇓λN , M⇓W , NW⇓ blame)) = {!!}
