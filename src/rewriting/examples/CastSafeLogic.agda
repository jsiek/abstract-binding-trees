...
compatible-app {Γ}{A}{B}{L}{M} ⊨L ⊨M γ = ⊢ℰLM
 ...
 ⊢ℰLM = ℰ-frame {F = □· (⟪ γ ⟫ M)} ⊢ℰL
                 (⊢ᵒ-∀-intro λ V → ⊢ᵒ-→-intro (⊢ᵒ-→-intro ⊢ℰVM))
  where
  ...
  ⊢ℰVM {V} = sucP⊢ᵒQ λ 𝒱Vsn →
       ...
       let 𝓟₁⊢ℰM : 𝓟₁ V ⊢ᵒ ℰ⟦ A ⟧ (⟪ γ ⟫ M)
           𝓟₁⊢ℰM = ⊢ᵒ-weaken (⊢ᵒ-weaken ⊢ℰM) in
       ℰ-frame {F = v ·□} 𝓟₁⊢ℰM
           (⊢ᵒ-∀-intro λ V → ⊢ᵒ-→-intro (⊢ᵒ-→-intro ⊢ℰVW))
   where
   ...
   ⊢ℰVW {V}{W} =
     ...
     let pres : 𝓟₂ (ƛ N′) W ⊢ᵒ preservation B (ƛ N′ · W)
         pres = ⊢ᵒ-∀-intro λ N → ⊢ᵒ-→-intro (Sᵒ⊢ᵒ λ {r →
               let ⊢▷ℰN′W = ⊢ᵒ-→-elim 𝒱W→ℰNW (⊢ᵒ-mono ⊢𝒱W) in
...
compatible-lambda {Γ}{A}{B}{N} ⊨N γ = ⊢ℰγλN
 ...
 ⊢ℰγλN =
   ...
   𝒱⇒ℰ (≡ᵒ⇒⊢ᵒ (⊢ᵒ-∀-intro λ W → ⊢ᵒ-→-intro G) (≡ᵒ-sym eq))
   where
   G : ∀{W} → ▷ᵒ 𝒱⟦ A ⟧ W ∷ 𝓖⟦ Γ ⟧ γ  ⊢ᵒ  ▷ᵒ ℰ⟦ B ⟧ (⟪ W • γ ⟫ N)
   G {W} =
     let ⊢𝒱W→ℰN[W] : 𝓖⟦ Γ ⟧ γ ⊢ᵒ (▷ᵒ (𝒱⟦ A ⟧ W) →ᵒ ▷ᵒ ℰ⟦ B ⟧ (⟪ W • γ ⟫ N))
         ⊢𝒱W→ℰN[W] = ⊢ᵒ-▷→{P = 𝒱⟦ A ⟧ W} (⊢ᵒ-mono (⊢ᵒ-→-intro (⊨N (W • γ))))
     in
     let ⊢▷𝒱W : ▷ᵒ 𝒱⟦ A ⟧ W ∷ 𝓖⟦ Γ ⟧ γ  ⊢ᵒ ▷ᵒ 𝒱⟦ A ⟧ W
         ⊢▷𝒱W = ⊢ᵒ-hyp in
     ⊢ᵒ-→-elim (⊢ᵒ-weaken ⊢𝒱W→ℰN[W]) ⊢▷𝒱W
...
compatible-inject {Γ}{G}{g}{M} ⊨M γ = ℰMg!
 ...
 ℰMg! : 𝓖⟦ Γ ⟧ γ ⊢ᵒ ℰ⟦ ★ ⟧ ((⟪ γ ⟫ M) ⟨ g !⟩)
 ℰMg! = ℰ-frame {F = □⟨ g !⟩} ⊢ℰM
            (⊢ᵒ-∀-intro λ V → ⊢ᵒ-→-intro (⊢ᵒ-→-intro ⊢ℰVg!))
  where
...
compatible-project {Γ}{H}{h}{M} ⊨M γ = ℰMh?
 ...
 ℰMh? = ℰ-frame {F = □⟨ h ?⟩} ⊢ℰM
            (⊢ᵒ-∀-intro λ V → ⊢ᵒ-→-intro (⊢ᵒ-→-intro ⊢ℰVh?))
  where
  ...
  ⊢ℰVh?{V} =
   ...
   let pres : 𝓟₁ (W ⟨ g !⟩) ⊢ᵒ preservation H ((W ⟨ g !⟩) ⟨ h ?⟩)
       pres = ⊢ᵒ-∀-intro λ N → ⊢ᵒ-→-intro (Sᵒ⊢ᵒ λ r → Goal r w ▷𝒱W) in
   ℰ-intro prog pres
   }
    where
    ...
    Goal {W}{G}{G}{g}{h}{W} (collapse{H} w′ g .h refl) w ▷𝒱W =
       ⊢ᵒ-▷→▷{_}{𝒱⟦ H ⟧ W} ▷𝒱W (⊢ᵒ-→-intro (𝒱⇒ℰ ⊢ᵒ-hyp))
