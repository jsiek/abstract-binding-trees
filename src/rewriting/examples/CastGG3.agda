...
𝒱⇒∀W′→∃W𝒱WW′ : ∀{c : Prec}{𝒫}{V}{V′}
   → 𝒫 ⊢ᵒ 𝒱⟦ c ⟧ V V′
     --------------------------------
   → 𝒫 ⊢ᵒ ∀ᵒ[ W′ ] ∀V′→∃V𝒱 c V V′ W′
𝒱⇒∀W′→∃W𝒱WW′ {c} {𝒫} {V} {V′} ⊢𝒱VV′ = 
      ...
      ((→ᵒI{P = ⇑ᵒ V′}
       ...
       ,ᵒ
        (Λᵒ[ V″ ] (→ᵒI{P = (V′ ⇓ V″)ᵒ} (→ᵒI{P = (Value V″)ᵒ }
         ...
         let ▷𝒱WV″ : 𝒫₁ ⊢ᵒ ▷ᵒ 𝒱⟦ gnd⇒ty G , A′ , G⊑A′ ⟧ W V″
             ▷𝒱WV″ = ▷→▷ ▷ℰWV′
                      (ℰ-converge Zᵒ (Sᵒ (Sᵒ Zᵒ)) (Sᵒ Zᵒ) λ V′₁ →
                      let 𝒫₂ = (W ⇓ V′₁)ᵒ ×ᵒ (Value V′₁)ᵒ
                                  ×ᵒ 𝒱⟦ gnd⇒ty G , A′ , G⊑A′ ⟧ V′₁ V″
                              ∷ ℰ⟦ gnd⇒ty G , A′ , G⊑A′ ⟧ W V′ 
                              ∷ 𝒫₁ in
                      ⊢ᵒ-sucP (proj₁ᵒ Zᵒ) λ W⇓V′₁ →
                       subst (λ X → 𝒫₂ ⊢ᵒ 𝒱⟦ gnd⇒ty G , A′ , G⊑A′ ⟧ X V″)
                             (⇓-determ W⇓V′₁ (⇓-value W w))
                             (proj₂ᵒ (proj₂ᵒ  Zᵒ))) in
          let 𝒱W!V″ : 𝒫₁ ⊢ᵒ 𝒱⟦ ★ , A′ , unk⊑ ⟧ (W ⟨ G !⟩) V″
              𝒱W!V″ = substᵒ (≡ᵒ-sym (𝒱-dyn-any G⊑A′))
                  (constᵒI w ,ᵒ (Zᵒ ,ᵒ ▷𝒱WV″)) in
          ⊢ᵒ-∃-intro-new
          (λ W → (V ⇓ W)ᵒ ×ᵒ (Value W)ᵒ ×ᵒ 𝒱⟦ ★ , A′ , unk⊑ ⟧ W V″)
          (W ⟨ G !⟩)
          (constᵒI (inj⇓ (⇓-value W w) w) ,ᵒ (constᵒI (w 〈 _ 〉) ,ᵒ
          𝒱W!V″)))))
          )
      }
 ... | yes refl =
