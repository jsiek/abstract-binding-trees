...
⇓⇒⇓𝒱 c M M′ = ∀ᵒ[ V′ ] (M′ ⇓ V′)ᵒ →ᵒ (Value V′)ᵒ
               →ᵒ (∃ᵒ[ V ] (M ⇓ V)ᵒ ×ᵒ (Value V)ᵒ ×ᵒ 𝒱⟦ c ⟧ V V′)

⇑⇒⇑ : Term → Term → Setᵒ
⇑⇒⇑ M M′ = (⇑ᵒ M′) →ᵒ (⇑ᵒ M)

ℰ-def : Prec → Term → Term → Setᵒ
ℰ-def c M M′ = (⇑⇒⇑ M M′) ×ᵒ (⇓⇒⇓𝒱 c M M′)
...
𝒱⇒⇓⇒⇓𝒱 {c} {𝒫} {V} {V′} ⊢𝒱VV′ = 
  ...
  let eq = (⇓-determ (⇓-value V′ v′) V′⇓W′) in
   subst (λ W′ → Value W′ ᵒ ∷ (V′ ⇓ W′) ᵒ ∷ 𝒫
               ⊢ᵒ ∃ᵒ[ W ] (V ⇓ W)ᵒ ×ᵒ (Value W)ᵒ ×ᵒ 𝒱⟦ c ⟧ W W′)
          eq
   (⊢ᵒ-∃-intro-new (λ W → (V ⇓ W) ᵒ ×ᵒ Value W ᵒ ×ᵒ 𝒱⟦ c ⟧ W V′) V
   (constᵒI (⇓-value V v) ,ᵒ (constᵒI v ,ᵒ (Sᵒ (Sᵒ ⊢𝒱VV′)))))
  )))

𝒱⇒⇑⇒⇑-pred : Setᵒ
𝒱⇒⇑⇒⇑-pred = ∀ᵒ[ c ] ∀ᵒ[ V ] ∀ᵒ[ V′ ] 𝒱⟦ c ⟧ V V′ →ᵒ ⇑⇒⇑ V V′

𝒱⇒⇑⇒⇑ᵒ : ∀{𝒫}
   → 𝒫 ⊢ᵒ 𝒱⇒⇑⇒⇑-pred
𝒱⇒⇑⇒⇑ᵒ {𝒫} = lobᵒ (Λᵒ[ c ] Λᵒ[ V ] Λᵒ[ V′ ] (→ᵒI (Goal{c}{V}{V′})))
  where
  Goal : ∀{c}{V}{V′}
    → 𝒱⟦ c ⟧ V V′ ∷ (▷ᵒ 𝒱⇒⇑⇒⇑-pred) ∷ 𝒫 ⊢ᵒ ⇑⇒⇑ V V′
  Goal {.★ , A′ , unk⊑} {V} {V′}
      with dyn? A′ 
  ... | no A′≢★ =
      ...
      (→ᵒI{P = ⇑ᵒ V′}
        (⊢ᵒ-intro λ { zero (⇑V′n , _) → ⇑zero
                    ; (suc n) (⇑V′sn , 𝒱W!V′ , ▷𝒱⇒⇑⇒⇑ , asms) →
         let 𝒱WV′ = ⊢ᵒ-elim ⊢▷𝒱WV′ (suc n) (𝒱W!V′ , (▷𝒱⇒⇑⇒⇑ , asms)) in
         let ⇑⇒⇑WV′ = ▷𝒱⇒⇑⇒⇑ (gnd⇒ty G , A′ , G⊑A′) W V′ {!!} {!!} 𝒱WV′ in
  ...
  ... | yes refl =
      ...
      (⊢ᵒ-intro λ { zero (⇑W′! , _) → ⇑zero
                  ; (suc n) (⇑inj ⇑W′ , 𝒱W!W′! , ▷𝒱⇒⇑⇒⇑ , asms) →
       let 𝒱WW′ = ⊢ᵒ-elim ⊢▷𝒱WW′ (suc n) (𝒱W!W′! , (▷𝒱⇒⇑⇒⇑ , asms)) in
       let ⇑⇒⇑WW′ = ▷𝒱⇒⇑⇒⇑ (gnd⇒ty G , gnd⇒ty G , Refl⊑) W W′ n ≤-refl 𝒱WW′
       in
       let ⇑W = ⇑⇒⇑WW′ n ≤-refl {- ⇑W′ -} in
       ⇑inj {!!} {- ⇑W -} }
      )))}
  Goal {.($ₜ _) , .($ₜ _) , base⊑} {V} {V′} =
     𝒱-base-elim Zᵒ λ{ c refl refl →
       (→ᵒI{P = ⇑ᵒ ($ c)} (⊢ᵒ-intro λ { .zero (⇑zero , asms) → ⇑zero})) }
  Goal {.(_ ⇒ _) , .(_ ⇒ _) , fun⊑ A⊑A′ B⊑B′} {V} {V′} = 
     𝒱-fun-elim Zᵒ λ{ N N′ refl refl 𝒱W→ℰNW →
       (→ᵒI{P = ⇑ᵒ (ƛ N′)} (⊢ᵒ-intro λ { zero x → ⇑zero})) }
...
𝒱⇒⇑⇒⇑ : ∀{c : Prec}{𝒫}{V}{V′}
   ...
   → 𝒫 ⊢ᵒ ⇑⇒⇑ V V′
𝒱⇒⇑⇒⇑ {c} {𝒫} {V} {V′} ⊢𝒱VV′ =
  appᵒ (instᵒ (instᵒ (instᵒ 𝒱⇒⇑⇒⇑ᵒ c) V) V′) ⊢𝒱VV′ 
...
𝒱⇒ℰ : ∀{c : Prec}{𝒫}{V}{V′}
   ...
   → 𝒫 ⊢ᵒ ℰ⟦ c ⟧ V V′
𝒱⇒ℰ {c} {𝒫} {V} {V′} ⊢𝒱VV′ =
    substᵒ (≡ᵒ-sym ℰ-stmt) (𝒱⇒⇑⇒⇑ ⊢𝒱VV′ ,ᵒ 𝒱⇒⇓⇒⇓𝒱 ⊢𝒱VV′)
...
compatible-app {Γ}{A}{A′}{B}{B′}{c}{d}{L}{L′}{M}{M′} ⊨L⊑L′ ⊨M⊑M′ γ γ′ =
  ...
  ToValue = Λᵒ[ V′ ] →ᵒI{P = ((⟪ γ′ ⟫ (L′ · M′)) ⇓ V′)ᵒ} (→ᵒI{P = (Value V′)ᵒ}
    (⊢ᵒ-sucP Zᵒ λ v′ →  (⊢ᵒ-sucP (Sᵒ Zᵒ) λ LM′⇓V′ → Goal v′ LM′⇓V′)))
    where
    Goal : ∀{V′}
       → Value V′
       → (⟪ γ′ ⟫ (L′ · M′)) ⇓ V′
...
compatible-inj-R{Γ}{G}{unk⊑}{M}{M′} ⊨M⊑M′ γ γ′ =
  ...
  Diverge = →ᵒI {P = ⇑ᵒ (⟪ γ′ ⟫ M′ ⟨ G !⟩)}
    (⊢⇑inj-inv
     let ▷⇑M = appᵒ (▷→ (monoᵒ (Sᵒ (ℰ-diverge (⊨M⊑M′ γ γ′))))) Zᵒ in
     {- Problem! have ▷ ⇑ M but need ⇑ M  -}
     {!⇑M!})
